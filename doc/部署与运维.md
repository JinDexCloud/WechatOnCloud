...
| Firefox 实例不启动 | 确认使用最新实例镜像和面板；修复版要求 APP_BIN=/usr/bin/firefox-esr 且实例类型为 Firefox。旧实例若被旧面板保存为微信，需新建 Firefox 实例 |
...
│   ├── Dockerfile         # KasmVNC base + 中文字体 + 多应用运行时依赖 + xdotool + IME 补丁（不打包微信本体）
...
│   ├── autostart          # openbox 会话启动：按 appType 拉起应用（崩溃自重启）+ 最小化窗口自动复原看守
